...
import { parseExecutionReport } from "./validate-execution-report.js";
...
export function evaluateExecution(requestValue: unknown, reportValue: unknown): EvaluationResult {
  ...
  const report = parseExecutionReport(reportValue);

  if (report.requestId !== request.id) {
    throw new Error(
      `Relatório de execução não corresponde ao pedido avaliado: esperado ${request.id}, recebido ${report.requestId}.`
    );
  }
  ...
  return {
    status,
    acceptancePassRate: acceptance.passRate,
    evidenceCoverage: acceptance.evidenceCoverage,
    efficiencyScore: score,
    efficiencyClass: classifyEfficiency(score),
    diagnostics
  };
}
